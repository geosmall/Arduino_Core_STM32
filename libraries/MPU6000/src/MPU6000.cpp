...
bool MPU6000::begin(SPIClass &spi, uint8_t cs_pin, uint32_t freq)
{
    ...
    // Attempt detection
    uint8_t detected = mpu6000SpiDetect(&gyro->dev);

    if (detected != MPU_60x0_SPI) {
        initialized = false;
        return false;
    }

    // Set hardware LPF to 256 Hz (DLPF disabled) by default
    gyro->hardware_lpf = BITS_DLPF_CFG_256HZ;

    // Initialize gyro and accelerometer
    mpu6000InitGyro(gyro);
    mpu6000InitAcc(acc);
...
bool MPU6000::readGyro(float &gx, float &gy, float &gz)
{
    if (!initialized) {
        return false;
    }
...
bool MPU6000::readAccel(float &ax, float &ay, float &az)
{
    if (!initialized) {
        return false;
    }
...
bool MPU6000::read6DOF(float &gx, float &gy, float &gz,
...
void MPU6000::setDLPF(uint8_t dlpf_cfg)
{
    if (!initialized || dlpf_cfg > 6) {
        return;
    }
...
void MPU6000::setGyroFSR(uint16_t fsr)
{
    if (!initialized) {
        return;
    }
...
void MPU6000::setAccelFSR(uint8_t fsr)
{
    if (!initialized) {
        return;
    }
    ...
    switch (fsr) {
        ...
        default:
            ...
            acc->acc_1G = 512;      // 512 LSB/g for ±16g (actually 2048 LSB/g)
            break;
    }
...
void MPU6000::setSampleRateDivider(uint8_t divider)
{
    if (!initialized) {
        return;
    }
    ...
    // Store divider in the device structure
    gyro->mpuDividerDrops = divider;

    // Write to SMPLRT_DIV register
    // Sample Rate = Internal_Sample_Rate / (1 + divider)
    // Internal rate is 1kHz when DLPF enabled, 8kHz when disabled
