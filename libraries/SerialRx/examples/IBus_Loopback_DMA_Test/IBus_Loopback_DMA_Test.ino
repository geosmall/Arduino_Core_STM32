 ...
 *
 * CI/HIL INTEGRATION:
 *   ./system/ci/aflash.sh libraries/SerialRx/examples/IBus_Loopback_DMA_Test --use-rtt --build-id
 */
...
void setup() {
  ...
  CI_LOG("  RX: USART1 @ 115200 baud (SerialRx DMA mode)\n\n");
  ...
  if (dma_mode_active) {
    CI_LOG("DMA mode: ENABLED\n");
  } else {
    CI_LOG("DMA mode: FALLBACK to interrupt (DMA not available for this UART)\n");
  }
...
void loop() {
  ...
  if (elapsed >= TEST_DURATION_MS) {
    ...
    CI_LOG("\n=== Test Complete ===\n");
    CI_LOGF("DMA Mode: %s\n", dma_mode_active ? "ACTIVE" : "FALLBACK");
    ...
    // Test end() cleanup
    rc.end();
    CI_LOG("SerialRx::end() called - cleanup complete\n");
