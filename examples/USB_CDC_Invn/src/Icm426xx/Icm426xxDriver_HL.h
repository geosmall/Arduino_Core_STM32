...
/** @brief Enable the 20bits-timestamp register access to read in a reliable way the
 *         strobed timestamp. To do that, the fine clock is forced enabled at some power cost.
 *  @param[in] s  Pointer to device.
 *  @return       0 on success, negative value on error.
 */
 ...
 *  @return              0 on success, negative value on error.
 *  @warning Timestamp register must be enabled before calling this API (see
 *           `inv_icm426xx_enable_timestamp_to_register` function)
 */
