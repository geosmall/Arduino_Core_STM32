...
# STM32CubeProgrammer upload script with J-Link support
# Based on STM32Tools stm32CubeProg.sh with added protocol 3 for J-Link
...
usage() {
  ...
  echo "## Options:"
  echo "##   For SWD, J-Link and DFU: no mandatory options"
...
case $PROTOCOL in
  ...
  3)
    PORT="JLINK"
    MODE="mode=UR"
    # Check for STM32CubeProgrammer J-Link library path bug workaround
    # STM32CubeProgrammer v2.20.0 has hardcoded relative path /../../../lib/libjlinkarm.so
    # which resolves incorrectly from typical install locations
