...
//STM32: Gyro/Accel ranges handled by IMU library (250 DPS, 2G)
...
//STM32: Gyro/Accel scale factors handled by IMU library
...
void setup() {
  ...
  // Arm motors at minimum (125µs)
  m1_command_PWM = 125;
  m2_command_PWM = 125;
  m3_command_PWM = 125;
  m4_command_PWM = 125;
  m5_command_PWM = 125;
  m6_command_PWM = 125;  // Not used, but keep for compatibility
...
void IMUinit() {
  ...
  // Configure gyro and accelerometer: 250 DPS, 2G, 2000 Hz ODR
  status = imu.ConfigureInvDevice(IMU::gpm2, IMU::dps250, IMU::accel_odr2k, IMU::gyr_odr2k);
...
void getIMUdata() {
  ...
  // Read raw IMU data (Accel X Y Z, Gyro X Y Z)
  std::array<int16_t, 6> imu_data;
  int status = imu.ReadIMU6(imu_data);

  if (status != 0) {
    // Use previous values on read failure
    return;
  }

  // Get sensitivity values for scaling
  float accel_sens = imu.GetAccelSensitivity();  // Returns sensitivity in LSB/g
  float gyro_sens = imu.GetGyroSensitivity();    // Returns sensitivity in LSB/(deg/sec)

  //Accelerometer - scale raw int16_t to g's
  AccX = imu_data[0] / accel_sens;  // g's
  AccY = imu_data[1] / accel_sens;
  AccZ = imu_data[2] / accel_sens;
  ...
  //Gyro - scale raw int16_t to deg/sec
  GyroX = imu_data[3] / gyro_sens;  // deg/sec
  GyroY = imu_data[4] / gyro_sens;
  GyroZ = imu_data[5] / gyro_sens;
  //Correct the outputs with the calculated error values
  ...
  //STM32: Zero magnetometer (ICM42688P has no magnetometer)
  MagX = 0;
  MagY = 0;
  MagZ = 0;
  MagX_prev = 0;
...
void calculate_IMU_error() {
  ...
  while (c < 12000) {
    ...
    if (imu.ReadIMU6(imu_data) == 0) {
      AccX  = imu_data[0] / accel_sens;
      AccY  = imu_data[1] / accel_sens;
      AccZ  = imu_data[2] / accel_sens;
      GyroX = imu_data[3] / gyro_sens;
      GyroY = imu_data[4] / gyro_sens;
      GyroZ = imu_data[5] / gyro_sens;
...
void calibrateAttitude() {
  ...
  for (int i = 0; i <= 10000; i++) {
    ...
    dt = (current_time - prev_time)/1000000.0; 
    getIMUdata();
    Madgwick6DOF(GyroX, -GyroY, -GyroZ, -AccX, AccY, AccZ, dt); //STM32: Use Madgwick6DOF (no mag)
...
void calibrateESCs() {
   ...
   */
   while (true) {
      ...
      getDesState(); //Convert raw commands to normalized values based on saturated control limits
      getIMUdata(); //Pulls raw gyro, accelerometer, and magnetometer data from IMU and LP filters to remove noise
      Madgwick6DOF(GyroX, -GyroY, -GyroZ, -AccX, AccY, AccZ, dt); //STM32: Use Madgwick6DOF (no mag) //Updates roll_IMU, pitch_IMU, and yaw_IMU (degrees)
      getDesState(); //Convert raw commands to normalized values based on saturated control limits
