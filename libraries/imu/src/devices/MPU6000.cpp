...
void MPU6000::enableDataReadyInt()
{
    bus_->writeReg(MPU_RA_INT_ENABLE, MPU_RF_DATA_RDY_EN);
    delayMicroseconds(15);
}

void MPU6000::disableDataReadyInt()
{
...
bool MPU6000::setGyroFSR(GyroFSR fsr)
{
    uint8_t fsr_code;
    float scale;
    switch(fsr) {
...
bool MPU6000::setAccelFSR(AccelFSR fsr)
{
    uint8_t fsr_code;
    float scale;
    switch(fsr) {
...
bool MPU6000::writeReg(uint8_t reg, uint8_t value)
{
    bus_->writeReg(reg, value);
    return true;
}

bool MPU6000::writeRegVerify(uint8_t reg, uint8_t value)
{
    bus_->writeReg(reg, value);
    delayMicroseconds(15);
