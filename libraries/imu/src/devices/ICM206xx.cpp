...
void ICM206xx::enableDataReadyInt()
{
    bus_->writeReg(MPU_RA_INT_ENABLE, MPU_RF_DATA_RDY_EN);
    delayMicroseconds(15);
}

void ICM206xx::disableDataReadyInt()
{
...
bool ICM206xx::setGyroFSR(GyroFSR fsr)
{
    uint8_t fsr_code;
    float scale;
    switch(fsr) {
...
bool ICM206xx::setAccelFSR(AccelFSR fsr)
{
    uint8_t fsr_code;
    float scale;
    switch(fsr) {
...
uint8_t ICM206xx::readReg(uint8_t reg)
{
    return bus_->readReg(reg);
}

bool ICM206xx::writeReg(uint8_t reg, uint8_t value)
{
    bus_->writeReg(reg, value);
    return true;
}

bool ICM206xx::writeRegVerify(uint8_t reg, uint8_t value)
{
    bus_->writeReg(reg, value);
    delayMicroseconds(15);
