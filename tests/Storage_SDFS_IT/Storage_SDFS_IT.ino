/**
 * Storage_SDFS_IT.ino - Integration test for Storage abstraction (SDFS)
 *
 * Category: Integration Test
 * Framework: AUnit
 * Hardware: SD Card
 *
 * Run with: ./system/ci/aflash.sh tests/Storage_SDFS_IT --use-rtt --build-id
 */
...
test(a_storage_init) {
  // Verify board config
  assertTrue(BoardConfig::storage.backend_type == StorageBackend::SDFS);
...
test(b_file_write) {
  ...
  assertTrue((bool)f);
  size_t written = f.println("AUnit Storage SDFS Test");
  f.close();
...
test(d_file_read) {
  ...
  f.close();
  assertTrue(content.startsWith("AUnit Storage SDFS Test"));
}
