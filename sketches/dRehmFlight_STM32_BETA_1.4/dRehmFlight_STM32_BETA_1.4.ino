...
//
//STM32 Port: BETA 1.4 - GPS support with optional UART DMA
...
//Uncomment to enable GPS (TinyGPSPlus library)
// #define USE_GPS
// #define USE_GPS_DMA  // Optional: Use UART DMA for GPS (reduces ~960 IRQ/sec to ~12)
...
void setup() {
  ...
  CI_LOG("dRehmFlight STM32 BETA 1.4\n");
  CI_BUILD_INFO();
...
void loop() {
  ...
  //#ifdef USE_GPS
  //printGPSdata();       //Prints GPS position, altitude, speed, satellites (expected: valid coords when fix acquired)
  //#endif
  ...
  #ifdef USE_GPS
    getGPSdata(); //Pulls GPS position, altitude, speed from TinyGPSPlus
  #endif
